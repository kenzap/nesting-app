(function attachNestDxfPreviewModalView(global) {
  ...
  function createDxfPreviewModal({ state }) {
    ...
    function syncActions() {
      ...
      dom.removeShapeLabel.textContent = selected?.visible === false ? t('preview.restore') : t('preview.hide');
    }
    ...
    function renderTabs() {
      ...
      dom.layerTabs.appendChild(makeTab(t('preview.all'), 'var(--text-muted)', null));
    ...
    async function openDXFPreview(fileId, filename) {
      ...
      dom.layerTabs.innerHTML = '';
      dom.fileMeta.textContent = t('common.loading');
      ...
      const hint = source === 'mock' ? ` · ${t('preview.previewHint')}` : '';
      dom.fileMeta.textContent = `${t('parts.shapeCount', { count: pv.shapes.length })} · ${t('parts.layerCount', { count: pv.layers.length })}${hint}`;
      renderTabs();
      renderSVG();
      renderList();
    ...
    return {
      ...
      refreshLabels() {
        syncActions();
        if (!dom.modal.classList.contains('open') || !pv.shapes.length) return;
        renderTabs();
        renderList();
      },
    };
