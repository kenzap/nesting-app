  ...
  try {
    ...
    if (inputPath && fs.existsSync(inputPath)) {
      try {
        ...
        Object.assign(exportSettings, inputData.settings || {});
    ...
    function collectLayerDefs(sheetStrips) {
      ...
      const addLayer = (name, color) => {
        ...
        // Allow real DXF layer colors to replace placeholder/default colors.
    ...
    function writeEntity(lines, entity, rotation, tx, ty, emitDebug = null, nextHandle = null) {
      ...
      if (entity.type === 'ARC' && entity.center && Number.isFinite(entity.radius)) {
        const center = transformPoint(entity.center, rotation, tx, ty);
        // dxf-parser returns angles in degrees already; add rotation offset and normalise to [0, 360)
      ...
      if ((entity.type === 'LWPOLYLINE' || entity.type === 'POLYLINE') && Array.isArray(entity.vertices) && entity.vertices.length >= 2) {
        // LWPOLYLINE is AC1015-only. For AC1014 compatibility, expand to LINE/ARC entities.
        ...
        for (let i = 0; i < count; i++) {
          ...
          if (Math.abs(bulge) > 1e-9) {
            ...
            const midX = (x1 + x2) / 2;
            const midY = (y1 + y2) / 2;
            // sagitta = distance from chord midpoint to arc midpoint; used to find center
    ...
    function buildDXF(sheetEntities, engravings, layerDefs, emitDebug) {
      ...
      // Seed BLOCKS with standard model/paper space blocks for better compatibility.
      L('0'); L('SECTION');
      L('2'); L('BLOCKS');
      L('0'); L('BLOCK');
      L('5'); L(nextHandle());
